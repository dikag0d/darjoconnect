...
class ReportPowerDetailScreen extends StatefulWidget {
  final Map<String, dynamic> laporan;

  const ReportPowerDetailScreen({super.key, required this.laporan});

  @override
  State<ReportPowerDetailScreen> createState() => _ReportPowerDetailScreenState();
}

class _ReportPowerDetailScreenState extends State<ReportPowerDetailScreen> {
  ...
  Future<void> updateStatus() async {
    ...
    final response = await http.post(url, body: {
      ...
      'tipe_laporan': 'listrik', // Mengganti tipe laporan menjadi listrik
    });

    print('Response status: ${response.statusCode}');
    print('Response body: ${response.body}'); // Tambahkan log untuk melihat respons
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status'] == 'success') {
        ...
        // Kembali ke halaman sebelumnya dan memberi tahu bahwa data telah diperbarui
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Detail Laporan Pemadaman Listrik',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailCard(
              title: 'Penyebab Pemadaman',
              content: widget.laporan['penyebab'] ?? '-',
            ),
            _buildDetailCard(
              title: 'Skala Pemadaman',
              content: widget.laporan['skala_pemadaman'] ?? '-',
            ),
            ...
            if (widget.laporan['image_url'] != null && widget.laporan['image_url'].isNotEmpty) 
              Column(
                ...
                children: [
                  ...
                  Image.network(
                    '${API.hostConnectUser}/${widget.laporan['image_url']}', // Hapus satu "uploads"
                    errorBuilder: (context, error, stackTrace) {
                      // Menampilkan pesan jika gambar gagal dimuat
            ...
            ElevatedButton(
              ...
              child: const Text('Update Status',
                style: TextStyle(color: Colors.yellow)), // Mengubah warna teks menjadi putih
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.black, // Menyesuaikan dengan tema pemadaman listrik
